...
function newCard() {
  ...
  let card = $("<div></div>").addClass('card');
    // .appendTo($('.grid'));
    // $('#popup .popup-content').empty();  // Kosongkan konten pop-up sebelumnya
    $('#popup .popup-content').append(card);  // Tambahkan card ke dalam pop-up
...
let onTopics = function(topics) {
  ...
  .click(() => { 
    initSubscribe({topicName: "/image_raw", topicType: "sensor_msgs/msg/Image"});
    initSubscribe({topicName: "/rosout", topicType: "rcl_interfaces/msg/Log"});
   })
  .text("TESTING")
...
// Viewer.onClose = function(viewerInstance) {
//   console.log("Closing viewer:", viewerInstance);
//   let topicName = viewerInstance.topicName;
//   let card = viewerInstance.card;

//   console.log("Card to remove:", card);

//   // Menghapus card dari pop-up
//   $('#popup .popup-content').masonry("remove", card);
//   $('#popup .popup-content').masonry("layout");

//   // Menyembunyikan pop-up setelah card dihapus (opsional)
//   $('#popup').fadeOut();

//   // Hapus viewer instance dari subscriptions
//   delete subscriptions[topicName].viewer;
//   delete subscriptions[topicName];
//   updateStoredSubscriptions();
// };
